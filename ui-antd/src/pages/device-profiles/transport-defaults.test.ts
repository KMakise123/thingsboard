...
describe('device profile configuration factories', () => {
  it('seeds MQTT with the upstream topic filters and JSON payload', () => {
    const config = createDeviceProfileTransportConfiguration(
  ...
  it('seeds SNMP with the upstream timeout/retries and keeps LWM2M a stub', () => {
    const snmp = createDeviceProfileTransportConfiguration(
      DeviceTransportType.SNMP,
    );
    expect(snmp).toMatchObject({ type: 'SNMP', timeoutMs: 500, retries: 0 });

    const lwm2m = createDeviceProfileTransportConfiguration(
      DeviceTransportType.LWM2M,
    );
    expect(lwm2m).toEqual({ type: 'LWM2M' });

    expect(
      createDeviceProfileTransportConfiguration(DeviceTransportType.DEFAULT),
    ).toEqual({
      type: 'DEFAULT',
    });
  });
  ...
  it('seeds the DEFAULT profile configuration and DISABLED provisioning', () => {
    ...
    expect(createDeviceProfileConfiguration(DeviceProfileType.SNMP)).toEqual({
      type: 'SNMP',
    });
