...
describe('tb http client', () => {
  ...
  beforeEach(() => {
    ...
    client = createTbHttpClient({
      fetchImpl: (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        return typeof next === 'function' ? next(calls[calls.length - 1]) : next ?? jsonResponse(200, {});
      }) as typeof fetch,
      language: () => 'zh-CN',
    });
  ...
  it('injects Authorization Bearer and Accept-Language', async () => {
    ...
    expect(headers.get('Authorization')).toBe(`Bearer ${tokenStore.getToken()}`);
  ...
  it('serializes query params and JSON body', async () => {
    ...
    expect(calls[0].url).toBe('/api/tenant/deviceInfos?pageSize=10&page=0&active=true');
    responses.push(jsonResponse(200, {}));
    await client.request('/api/device', { method: 'POST', body: { name: 'd1' } });
  ...
  it('login and token endpoints are exempt from auth header and refresh', async () => {
    responses.push(jsonResponse(401, { status: 401, message: 'Invalid username or password', errorCode: 10 }));
    await expect(
      client.request('/api/auth/login', { method: 'POST', body: { username: 'a', password: 'b' } }),
    ).rejects.toMatchObject({ status: 401, detail: 'Invalid username or password' });
    const headers = new Headers(calls[0].init?.headers);
  ...
  it('401 triggers refresh once and replays with the new token', async () => {
    const expired401 = jsonResponse(401, { status: 401, message: 'Token has expired', errorCode: 11 });
    const newToken = makeJwt('tenant@tb');
    responses.push(
      expired401,
      jsonResponse(200, { token: newToken, refreshToken: makeJwt('tenant@tb', 604800) }),
  ...
  it('concurrent 401s share one refresh (single-flight) and each replays', async () => {
    ...
    const auths = calls.slice(3).map((c) => new Headers(c.init?.headers).get('Authorization'));
  ...
  it('refresh failure clears tokens and fires onUnauthorized', async () => {
    ...
    responses.push(
      jsonResponse(401, { status: 401, message: 'expired', errorCode: 11 }),
      jsonResponse(401, { status: 401, message: 'Invalid refresh token', errorCode: 10 }),
    );
    await expect(failing.request('/api/auth/user')).rejects.toMatchObject({ status: 401 });
    expect(calls.filter((c) => c.url === '/api/auth/token')).toHaveLength(1);
    expect(onUnauthorized).toHaveBeenCalledTimes(1);
    expect(onUnauthorized).toHaveBeenCalledWith(expect.objectContaining({ source: 'http' }));
  ...
  it('refresh endpoint own-401 does not recurse', async () => {
    responses.push(
      jsonResponse(401, { status: 401, message: 'expired', errorCode: 11 }),
      jsonResponse(401, { status: 401, message: 'refresh expired', errorCode: 11 }),
    );
    await expect(client.request('/api/auth/user')).rejects.toMatchObject({ status: 401 });
  ...
  it('429 retries with random backoff and succeeds', async () => {
    ...
    try {
      responses.push(
        jsonResponse(429, { status: 429, message: 'Too many requests for current tenant!', errorCode: 33 }),
        jsonResponse(429, { status: 429, message: 'Too many requests for current tenant!', errorCode: 33 }),
  ...
  it('429 gives up after max retries and normalizes the error', async () => {
    ...
    try {
      for (let i = 0; i < 5; i++) {
        responses.push(jsonResponse(429, { status: 429, message: 'rate limited', errorCode: 33 }));
      }
      const promise = client.request('/api/devices');
      const assertion = expect(promise).rejects.toMatchObject({ status: 429, titleKey: 'tb.error.tooManyRequests' });
  ...
  it('times out after 10s and surfaces a timeout error', async () => {
    ...
    try {
      ...
      const promise = client.request('/api/devices');
      const assertion = expect(promise).rejects.toMatchObject({ status: 0, titleKey: 'tb.error.timeout' });
  ...
  it('204/empty body resolves to undefined', async () => {
    ...
    await expect(client.request('/api/device/some-id', { method: 'DELETE' })).resolves.toBeUndefined();
  });

  it('normalizes error body into ServerError fields', async () => {
    responses.push(jsonResponse(403, { status: 403, message: 'You don\'t have permission', errorCode: 20 }));
    await expect(client.request('/api/device/x', { method: 'DELETE' })).rejects.toMatchObject({
      status: 403,
      errorCode: 20,
