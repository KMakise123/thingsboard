/**
 * Entry checkpoint for the editor suite — §3.1 取消退出 (prevX semantics).
 * Generic core hoisted from pages/dashboards/editor/contract in M8 wave F.
 *
...
export interface UseEditorEntryCheckpointOptions<T extends object = object> {
...
export function useEditorEntryCheckpoint<T extends object>({
  session,
  enabled,
}: UseEditorEntryCheckpointOptions<T>): EditorEntryCheckpoint {
  const handleRef = useRef<ReturnType<EditorSession<T>['checkpoint']> | null>(
    null,
  );
