 ...
 * - breadcrumb: parents come from the matched route tree (menu names), the
 *   leaf shows the page's real name. Detail pages pass `breadcrumbLabel`
 *   (the entity name, with the same `entity?.name ?? id` fallback the
 *   header title uses) so the dynamic segment resolves in the same frame —
 *   no store indirection needed at page level.
...
export function buildBreadcrumbItems(
  ...
  const named = matches.filter((match) => match.route.name);
  return named.map((match, index) => {
    const isLeaf = index === named.length - 1;
    const item: BreadcrumbItem = {
      title:
        isLeaf && breadcrumbLabel
          ? breadcrumbLabel
          : menuLabelFor(formatMessage, match.route.name as string),
      href: isLeaf ? undefined : match.pathname,
    };
