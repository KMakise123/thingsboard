...
test('correct credentials land on the device list', async ({ page }) => {
  ...
  await expect(
    page
      .getByRole('main')
      .getByText(/设备|Devices/i)
      .first(),
  ).toBeVisible();
});

test('wrong credentials surface the backend error verbatim, no blank page', async ({
  page,
}) => {
...
test('logout clears storage and returns to the login page', async ({
  page,
}) => {
  await uiLogin(page, 'tenant@thingsboard.org', 'tenant');
  await expect(page).toHaveURL(/\/devices/);
  ...
  await page
    .locator('.ant-avatar, header .anticon-user, header img[alt="user"]')
    .first()
    .hover();
