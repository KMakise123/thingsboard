...
4. **产物形态全在前端侧对齐，后端 4 处路径集合（WebConfig 正则 + security 三处）零改动**：`outputPath: 'target/generated-resources/public'`（构建前自动 clean）、`hash: true` + `publicPath: '/'`、browser history、**删脚手架 `exportStatic`**（多路由 HTML 与 `forward:/index.html` 回退冗余）；~~`staticPathPrefix: 'assets/'`~~〔M6 修订 2026-09-03：该键不存在于用户级 umi config（utoopack 内部选项，无 preset-umi schema 注册，顶层设置触发 `Invalid config keys` 断言）。实测产物布局无冲突：index.html 与 hash 资产均在根、`public/` 内容按原结构映射（项目自有的 `public/static/` 瓦片资源本就归属后端豁免前缀 `/static/**`），后端 `forward:/index.html` 回退与之兼容〕、`define: { __APP_VERSION__: package.json version }`（版本号节奏另票）。
