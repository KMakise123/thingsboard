...
- [x] 网关：设备列表变体（gateway profile 过滤），行操作对齐〔M5 ✅ 收口——按 ui-ngx 真实形态交付为系统仪表盘页 `/entities/gateways`（GET /api/resource/dashboard/system/gateways_dashboard.json，真机 200），default 态 = Gateway list 实体表；下钻 states（gateway_details / connectors / logs 等 33 个）可达，冷门 gateway_widgets.* / markdown_card 按锚点外口径渲染占位；期间发现并修复 WS 订阅缺陷见 §3.10 注与 BCR C-19/C-20〕
...
- [x] CU 登录：菜单四项、无 CRUD 按钮、数据只含所属客户作用域〔M2 ✅ 现存项；四项中告警（M3 ✅）/ 仪表盘（M5 ✅：四菜单含「仪表盘」，列表 = 创建时间 + 标题双列只读、无操作列 / 批量 / 导入；只读页 toolbar 仅 export + 全屏 + timewindow）随各自里程碑补齐〕
...
> M5 验收注（2026-09-03）：6/6 勾。真机走查（TA tenant@thingsboard.org + CU 测试号）逐项通过；门禁三绿（lint / vitest 770 / build）。验收中修复 3 枚 WS / 渲染缺陷（gateways 页 WS 帧被后端 1007 拒后误判认证失败致整页登出、timeseries 订阅缺 latestValues 被后端 NPE 拒订、realtime 图表流式新点被订阅时刻的静态窗口上界过滤——均带渲染级测试，6fd0f8c3a7）；后端契约歧义登记 BCR C-19 / C-20。环境事故一次：dev server（utoopack）资产服务损坏（/umi.js 返回 HTML fallback）致页面永久 loading，按崩溃剧本杀旧起新一次（8000 单实例保持）。

- [x] 列表操作集对齐：export / import JSON、assign to customers（单 + 批）、make public / manage customers、删除〔M5 ✅：export 下载 Thermostats.json 实查（states/widgets/entityAliases 全量、id/tenantId 已剥、恒 includeResources）；import 往返 = 改标题导出件导入入列 → 删除还原；assign 单个（AssignCustomerModal）+ 批量（2 仪表盘 → Customer B，进度弹窗「成功 2 失败 0」）+ 批量取消分配，customersTitle 列即时变化；make public 弹窗生成 `…/dashboard/{id}?publicId={publicCustomerId}` 链接并勾选公开列（v1 仅生成展示，匿名页遗留），make public/private 菜单项按状态切换；manage customers 对话框三动作（update 全集 / add / remove）逐项真机核验，公开态移除 Public customer 即回私有（TB 语义）；删除二次确认框标题插值正确；验收数据全部还原〕
- [x] 只读页：state 切换（default / entity 两 controller、URL base64 state 契约沿用 #6）；全局 timewindow（presets + 自动刷新）；toolbar 全量（编辑器入口按原则 3 隐藏）〔M5 ✅：entity controller（Thermostats）Edit location（openDashboardState）导航 + 面包屑插值「Thermostat T1」+ `?state=` 深链直达 / 刷新恢复同 state；default controller（usage）出 stateOptions 下拉；timewindow 25 档预设 + 实时/历史 + 自定义起止 RangePicker + 清除，换档后聚合间隔自动换算（1m 档 1s → 1h 档 30s）、自动刷新 = WS 流式（见修订记录口径）；toolbar = timewindow / dashboards-select / export / 收起 / 全屏，编辑器入口（edit FAB / update-image）零渲染，原则 3 的 v2 提示以占位组件文案形态在场；标题按 settings.showTitle 条件渲染（Thermostats showTitle=false 不渲染 dashboard 标题，壳层 PageContainer 标题属应用壳形态）〕
- [x] `dashboard/:id` 全屏 single-page 模式〔M5 ✅：toolbar 全屏按钮进 `/dashboard/:id`（单数，layout:false），无应用壳；全屏态按钮呈「退出全屏」形态（ui-ngx parity），退出回 `/dashboards/:id` 且保留 `?state=`〕
- [x] `usage` 页随仪表盘渲染交付〔M5 ✅：default state main（api_usage 卡占位，预期）+ right（4 张 hourly 图）双列布局真机核验，rule-engine 图真实出数；31 个 widget 定义中其余 26 个位于实体下钻 states，依赖 api_usage Angular 卡的交互，v1 占位后不可达（登记口径）；widget 标题渲染 `{i18n:api-usage.*}` 原始键（i18n 资产在 widgetType descriptor 内，v1 不解析，登记）〕
- [x] 布局：断点覆盖 / mobile 单列（#6 RGL 完全受控模式）〔M5 ✅：375px 视口 Rule Engine 仪表盘 3 widget 单列栈（同 x 同宽纵向堆叠）；usage 移动端 main 列独显、right 列经 toolbar right-circle 切换展开为单列栈；isDraggable/isResizable 关闭〕
- [x] **widget 验收锚点**：CE 自带全部 demo 仪表盘渲染无「暂未支持」占位；四类先行（时序折线 / 柱状、值卡片、实体表格、gauge）→ 其余按 demo 需要补齐；锚点外冷门类型允许占位（登记于第 8 节）〔M5 ✅：Thermostats / Firmware / Software / Rule Engine Statistics 四 demo 逐个扫描零「暂未支持」；Firmware default = Device list 实体表 + 4 张 html_value_card（Device Waiting/Updating/Failed/Updated），4 状态下钻表行数与卡片计数一致（全 0=0）；usage / gateways 系统页 widget 渲染见上；冷门占位（gateway_widgets.*、markdown_card 等）fqn 明示 + 「将在后续版本提供」；遥测新鲜度抽测 = POST /api/v1/{token}/telemetry 推 temperature/humidity，WS update 帧 ~81ms 到达页面、图表渲染出新点（本项验收揪出缺陷 #3），推完数据已删〕
