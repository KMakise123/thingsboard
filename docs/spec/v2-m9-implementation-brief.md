...
- **P1**（K，ADR 附录 A）：Sucrase 编译错/运行错行号与源码行号一致（含多行 JSX 用例）；不一致则评估 sourceMap 路线。
- **P2**（K）：require shim 传宿主 React 单实例 + antd 组件在编译产物内可用（`$$typeof` 同源断言）。
- **P9**（D）：TB Angular widget JSON 导入 → badge + 占位链路。
- **P10**（K/D）：CSS 前缀器对 @media/@keyframes 的正确性（K style-scope 单测）；includeResources 导出 round-trip（D import-export）。
- 备注：P5 撤销属性测试已有 session.test 锚；P8 descriptor 预算随 512KB 警告测试顺手取证。
