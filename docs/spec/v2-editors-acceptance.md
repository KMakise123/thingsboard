...
- [x] **scada 置顶机制（M11 补走查，2026-09-05）**：scada 布局下添加 widget 抽屉的 `scadaFirst` 参数透传已交付并实测——两路请求 `/api/widgetTypes?…&scadaFirst=true`、`/api/widgetsBundles?…&scadaFirst=true`（performance 取证，scada 布局盘 `layoutType: scada` API 复核）。**口径注记（主会话裁决）**：「scada 类目置顶可见」受 M7 抽屉 registry-only 数据源限制——抽屉分组来自 fork widget registry（按字母序 Alarm widgets/Analogue gauges/Cards/通用/Input widgets…），scada 符号类型不在 registry，类目无承载故置顶不可见；完整目击待抽屉数据源改造（随 scada 域迭代）。参数透传半边勾账，置顶可见不冒勾〔证据见 v2-m11-browser-walkthrough.md 步骤 9；原「缺口登记（V 波）」行由本行接替〕
...
- [x] 边界（M11 走查，2026-09-05）：SCADA 符号编辑器页（`/resources/scada-symbols`，SVG 结构 / tag / 行为元数据编辑 + pan/zoom）**已随资源库子系统交付**（`v2-subsystems-acceptance.md` §3.3 全段勾账，编辑器页可进可编辑 readonly 边界在）；仪表盘内 symbol 实例边界实测：① 「不能改 SVG 结构」成立——配置面板五分组（数据/外观/Widget 卡片/操作/布局）全通用表单，全编辑器无任何 SVG 结构编辑路径；② 「只能换符号 / 绑设备 / 绑对象」在 fork 当前**无承载**：专用符号实例 basic editor 注册位预留未实现（`basic-config.tsx:8-9` 注释自述），实例以占位三态渲染（「暂不支持（Angular 部件）」+ fqn 徽标，scada 渲染器缺口 = 资源库 spec §3.8 登记）——**换符号/绑设备/绑对象能力随渲染器 + 专用编辑器后续交付，不冒勾**〔证据见 v2-m11-browser-walkthrough.md 步骤 10〕
...
- 2026-09-05：**M11 3V 波解锁两条补走查（§3.2 scada 置顶缺口行 + §3.6 边界行回写）**。§3.2：原「缺口登记」行由新行接替——scada 布局下抽屉 `scadaFirst=true` 参数透传两路（widgetTypes/widgetsBundles）performance 实测勾账；「scada 类目置顶可见」受 M7 抽屉 registry-only 数据源限制（registry 无 scada 类目），按主会话裁决口径登记、完整目击待抽屉数据源改造。§3.6：边界行勾账——「不能改 SVG 结构」真机成立（配置面板五分组零 SVG 编辑入口）；「换符号/绑设备/绑对象」在 fork 无承载（专用 basic editor 注册位预留未实现 + 实例以占位三态渲染，scada 渲染器缺口随资源库 spec §3.8），不冒勾。证据全文见 [v2-m11-browser-walkthrough.md](./v2-m11-browser-walkthrough.md)。
