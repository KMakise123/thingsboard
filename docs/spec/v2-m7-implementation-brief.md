...
- **P7**（V，✅ 2026-09-04 取证落地，回归测试留存 `canvas/memo-boundary.perf.test.tsx`）：120-widget 编辑网格 + WIDGET_REGISTRY 钉桩渲染计数探针。结论：**memo 边界成立**——对单一 widget `session.write(updateWidgetConfig)` 后，全画布内容重渲染 delta = 1（仅目标 widget；其余 119 个 cell 计数纹丝不动），连续两次写只增加目标侧计数；点选（选中态走 wrapper 通道，ADR 0004 §2）零内容重渲染。happy-dom 无 layout/paint 管线，fps 不可测——按诚实证据原则交重渲染拓扑。装置性发现两条（非组件缺陷）：① 挂载后首次 act 刷新会一次性冲掉 lazy registry 桩的 Suspense retry 待定工作（每 cell 多计 1 次、仅一次；plain 非 lazy 桩对照零误渲染），测试以 warm-up write+undo 冲洗后按基线快照计 delta；② 装置若在 host 重渲染时重建 QueryClient/内联 props，react-query 的 context 变更会穿透 memo——真实 shell 单例 client + 稳定 props 不受影响。
